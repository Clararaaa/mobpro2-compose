...
import com.indraazimi.mobpro2m.notify.FcmServices.Companion.NOTIFICATION_ID
...
class FcmServices : FirebaseMessagingService() {
    ...
    override fun onNewToken(token: String) {
        Log.d("FCM", "Token bary: $token")
    }
}

@RequiresPermission(Manifest.permission.POST_NOTIFICATIONS)
private fun sendNotification(
    context: Context, title: String, body: String, url: String
) {
...
private fun getPendingIntent(context: Context, url: String): PendingIntent {
    ...
    intent.putExtra(FcmServices.KEY_URL, url)
